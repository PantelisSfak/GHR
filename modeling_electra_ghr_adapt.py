...
from bottleneck_adapter_modeling import Adapter
...
class ElectraSelfOutput(nn.Module):
    def __init__(self, config):
        ...
        #####add_adapter_implementation
        use_adapters = True
        reduction_factor = 16
        configuration = 1024#cannot inherit the pretrained model info
        
        if use_adapters:
          adapter_class = Adapter
          #Create a bottleneck adapter and put it in nn.Moduledict to create the adapter layer
          adapter = adapter_class(
                  adapter_name="bottleneck_adapter",
                  input_size = configuration,
                  down_sample = int(configuration // reduction_factor),
                  config = AdapterConfig,
            )#AdapterConfig is the configuration for Bottleneck adapter
          self.adapters["bottleneck_adapter"] = adapter


    def forward(self, hidden_states, input_tensor):
        hidden_states = self.dense(hidden_states)
        hidden_states = self.dropout(hidden_states)
        #from here the changes for adapters
        use_adapters = True
        layer_norm = self.LayerNorm

        if use_adapters:
          input_hidden_states = hidden_states
          adapter_layer = self.adapters["bottleneck_adapter"]

          "before the adapter"
          hidden_states, query, residual = adapter_layer.pre_forward(hidden_states, input_tensor, layer_norm)
          "adapter_layer"
          layer_output = adapter_layer(
                    hidden_states, residual_input=residual, output_gating=False
                )

          hidden_states = layer_output[0]#adapter_layer forward returns
          '''
          post_forward : Performs computations after the forward pass of the adapter block(s). 
          This e.g. includes applying the residual
          connection and layer norm if configured in this way.
          '''
          hidden_states = adapter_layer.post_forward(hidden_states, input_hidden_states, input_tensor, layer_norm)
        else: 
